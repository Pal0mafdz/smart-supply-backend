...
export const closeSession = async (req: Request, res: Response) => {
  try {
    const { realClosingAmount, note, date } = req.body;

    // Fecha por default: hoy (yyyy-mm-dd)
    const today = new Date();
    const y = today.getFullYear();
    const m = String(today.getMonth() + 1).padStart(2, "0");
    const d = String(today.getDate()).padStart(2, "0");
    const sessionDate: string = date || `${y}-${m}-${d}`;

    // Solo cerrar la sesión, sin Excel
    const cashSession = await CashSession.findOneAndUpdate(
      { date: sessionDate },
      {
        status: "cerrada",
        closingAmount: realClosingAmount,
        note: note ?? "",
        closedAt: new Date(),
      },
      { new: true }
    ...
    if (!cashSession) {
      return res.status(404).json({
        message: "No se encontró sesión de caja para esa fecha",
      });
    }
